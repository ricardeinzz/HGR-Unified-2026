...
% Obtain the root directory of the project (one level above 'CNN/')
...
if isequal(DATASET_FORMAT, 'mat')
    ...
    datastoreOut = fullfile(rootDir, 'Datastores');     % output folder
elseif isequal(DATASET_FORMAT, 'json')
    trainingDir  = 'trainingJSON1';
    datastoreOut = fullfile(rootDir, 'Datastores1');    % output folder
else
...
for i = 1:size(usersSets, 1)
    ...
    parfor j = 1:length(users)
        ...
        % Get user samples using unified logic
...
function data = generateFramesNoGesture(signal, numWindows)
    % Calculate the number of windows that can be extracted natively
    numWindowsNative = floor((length(signal) - Shared.FRAME_WINDOW - 100) / Shared.WINDOW_STEP) + 1;
    if numWindowsNative < 1
       numWindowsNative = 0; 
    end
    
    % Calculate if signal needs filling (if requested windows exceed native capacity)
    numWindowsFill = numWindows - numWindowsNative;
    if numWindowsFill > 0
        % Get a nogesture portion of the sample to use as filling
        fillingLength = numWindowsFill * Shared.WINDOW_STEP;
        % Guard against very short signals by wrapping the signal safely
        if fillingLength <= length(signal)
            filling = signal(1:fillingLength, :);
        else
            filling = repmat(signal, ceil(fillingLength/length(signal)), 1);
            filling = filling(1:fillingLength, :);
        end
        % Fill the signal array before creating frames
